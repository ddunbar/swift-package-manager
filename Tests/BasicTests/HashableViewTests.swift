...
class HashableViewTests: XCTestCase {
    func testBasics() {
        class Airport {
        ...
        func whereCanIGo(from airport: Airport) -> [Airport] {
            func makeView(for airport: Airport) -> HashableView<Airport, String> {
                return HashableView(airport, accessor: { $0.name })
            }
            let closure = transitiveClosure([makeView(for: airport)]) {
                return $0.item.destinations.map(makeView)
            }
